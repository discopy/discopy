...
from discopy.quantum import (
    ...
    MixedState, Discard, bit, sqrt, CX, H, SWAP, X, Y, Z, CZ)
...
mixed_circuits = [
    ...
    (Ket(0, 0) >> CZ >> Rx(6.24) @ Id(1) >> Id(1) @ Rx(6.09) >> H @ Id(1))
    @ (Ket(0, 0) >> H @ Id(1) >> Id(1) @ H >> CZ) >> Id(2) @ Measure(2)
]
...
def test_consistent_eval(c):
    ...
    doubled_result = (pure_result.conjugate(diagrammatic=False)
                      @ pure_result)
    np.allclose(doubled_result, mixed_result, rtol=tol, atol=tol)
...
@pytest.mark.parametrize('c', mixed_circuits)
@pytest.mark.parametrize('backend', ['jax', 'tensorflow', 'numpy'])
def test_positive_eigenvalues(c, backend):
    Tensor.set_backend(backend)
    tn.set_default_backend(backend)
    dm = c.eval(mixed=True, contractor=tn.contractors.auto).array
    d = int(np.sqrt(2 ** len(dm.shape)))
    dm = dm.reshape(d, d)
    eigvals = np.array(np.linalg.eigvals(dm), dtype=float)
    assert all(eigvals > -1.e-8)
