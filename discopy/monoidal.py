...
class Ty(cat.Ob, FreeMonoid):
    ...
    Note
    ----
    The variadic ``Ty('x', 'y')`` above is the user-friendly API. Internally,
    the free-category machinery rebuilds types by keyword, passing the path of
    generators as ``inside=``; both forms agree.

    >>> assert Ty('x', 'y') == Ty(inside=(Wire('x'), Wire('y')))
    ...
    def cast_wire(self, x: str | cat.Ob) -> cat.Ob:
        ...
        A plain :class:`cat.Ob` is upgraded to ``Wire(x.name)`` for subclasses
        whose generators are plain :class:`Wire`, so that old dumps and pickles
        (which stored an uncoloured :class:`cat.Ob` as the generators) still
        load.
        """
...
class PRO(Ty):
    ...
    ----------
    inside : int or tuple
        The length of the PRO type, or the path of generators to count.
