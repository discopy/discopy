...
- [ ] `ask()` retries once without the `reasoning` field when the gateway's
  400 says reasoning can't be disabled, instead of crashing the job —
  keeps the disable-by-default behavior for models that support it
