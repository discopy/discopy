...
def normalize_svg(path):
    ...
    text = re.sub(
        r'<!-- Generated by graphviz.*?-->', '', text, flags=re.DOTALL)
    text = re.sub(r'="[^"]*"', '="#"', text)
    text = text.replace(' xml:space="#"', '')
    text = re.sub(
        r'<g id="#"><a xlink:title="#">\s*(<text.*?</text>)\s*</a>\s*</g>',
        r'\1', text, flags=re.DOTALL)
    text = re.sub(
        r'<g id="#"><a xlink:title="#">\s*</a>\s*</g>',
        '', text, flags=re.DOTALL)
    return re.sub(r'\n\s*\n+', '\n', text)
...
def save_drawing(path, save, replace=None, tol=DEFAULT['tol']):
...
def savefig(path, replace=None, tol=DEFAULT['tol']):
    ...
    save_drawing(path, save, replace=replace, tol=tol)
