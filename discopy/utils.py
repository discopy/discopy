...
def factory(cls):
    ...
    within the subclass, by setting ``cls.factory = cls``.
    ...
    Note
    ----
    The factory method pattern (`FMP`_) is used all over DisCoPy. The arrow
    factory is available both as ``cls.factory`` and, for backward
    compatibility, as the ``cls.ar`` class property defined on
    :class:`Category`.
