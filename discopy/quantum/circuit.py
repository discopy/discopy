...
def jacobian(circ, variables):
    """
    Prepare a matrix of circuits corresponding to the Jacobian of the given
    circuit over the given variables which are considered as the inputs.
    The ordering of the outputs follows the circuit matrix from the top to the
    bottom, similarly the ordering of the variables coicide with the
    corresponding parameter (variables).
    :param circ: The input circuit.
    :param variables: The sympy variables or their names to be used for
    the differentiation.
    :return: A matrix (numpy) of circuits.
    """
    ...
    if isinstance(circ, Circuit):
        ...
        dvs = [(circ >> Bra(*bra)).grad(v) for bra, v in
               product(bras, variables)]
    else:
        # Ideally the support for tensors can be implemented here.
        raise NotImplementedError
    dvs1 = np.empty(len(dvs), dtype=np.object)
    dvs1[:] = dvs   # Avoid numpy iterate over objects
