...
Context: this branch lands the tensor carrier, stage of a 12-branch split of
daydream6728/discopy#2 ("The great purification: property matrix over every
carrier"), publishing pre-built, already-reviewed-and-tested work from
`daydream6728/discopy:split/4-tensor` onto `discopy/discopy` as a
ready-for-review pull request stacked on `split/4-matrix` (Tensor subclasses
Matrix, so this stacks on the matrix stage rather than directly on stage 3).
...
- [x] Open PR stacked on split/4-matrix using the repo's Why/What/How template
