...
- a generator is defined once, in the module that introduces it: `symmetric.Diagram` declares `swap_factory` with the `Generator` decorator as a method returning `Swap`, and every subclass decorated with `@factory` gets its own `Swap` built once from those of its bases, e.g. `closed.Swap = closed.Diagram.swap_factory` is a `markov.Swap` that is a `closed.Permutation` and a `closed.Box`; a level only writes a generator by hand when it adds behaviour to it, and declares it again
