...
class Ty(Pregroup, biclosed.Ty):
    ...
    def unwind(self) -> Ty:
        ...
        A type generated by something other than a :class:`Wire`, e.g. the
        integers inside a :class:`PRO` or a ``Dim``, has no winding to undo.

        >>> assert PRO(1).unwind() == PRO(1)
        """
        assert_isatomic(self)
        obj, = self.inside
        return self.ar(obj.unwind()) if isinstance(obj, Wire) else self
