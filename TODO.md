...
- [ ] Route the term factories through `Generator` the same way, so `cartesian` pulls `TermBase`, `Constant` and `Variable` and declares only its `Application`.
- [ ] `Projection` generators in `cartesian`: `abc.CartesianCategory.projection` derived from copy and discard, the free generator declared with `@Generator`, mapped by `Functor`, with tests.
