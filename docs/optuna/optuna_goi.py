...
touched here; retrain the winner at the full recorded widths, for
longer, and evaluate it once.
 
Trials report to the study after every epoch, so a median pruner kills
bad configurations early; the study lives in ``--storage`` (a sqlite file
by default) and resumes if the process is restarted. ``--gpus`` runs one
worker process per GPU on this host, sharing one study, capped at however
many are actually visible -- ``--gpus 8`` on a 3-GPU box just uses the 3.
For a search spanning *several hosts* sharing a mounted filesystem, launch
``--gpus 1`` on each and point them at the same path instead of a
``sqlite:///`` URL, which switches to a journal-file backend (see
:func:`make_storage`) since sqlite's locking is unreliable over NFS.
"""
...
def make_storage(uri: str):
    """
    An RDB storage for a ``scheme://`` URI (e.g. ``sqlite:///...``), or a
    journal file at ``uri`` otherwise.

    The journal backend uses a symlink lock rather than sqlite's ``fcntl()``
    locking, which is unreliable on NFS -- so several machines can point
    ``--storage`` at the same path on a filesystem they all mount and join
    one study, which a shared sqlite file cannot guarantee across hosts.
    """
    if "://" in uri:
        return uri
    return JournalStorage(JournalFileBackend(uri, JournalFileSymlinkLock(uri)))
...
def objective(trial, arguments, train_clues, train_targets,
    ...
    model = zoo.GoISolver(SEARCH_WIDTHS, rounds=rounds).to(device)
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--storage", default="sqlite:///optuna_a_goi.db",
                        help="a sqlite/postgres URL, or a plain path to use "
                             "a journal file instead (for studies shared "
                             "across machines over a mounted filesystem)")
    ...
    study = optuna.create_study(
        study_name=arguments.study_name, storage=make_storage(arguments.storage),
