...
- [x] Make `monoidal.Colour` transparent by default: `config.TRANSPARENT`,
      `monoidal.transparent` in place of `monoidal.white`, `rigid.Wire`.
- [x] Remove the "if white then transparent" special cases in
...
- [x] Update the tests that assert white is the neutral background and add
      regressions for an explicitly white region and spider.
- [x] Add a `CHANGELOG.md` entry, run `pflake8 discopy` and the full suite.
