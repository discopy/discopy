...
- [ ] Extract the operand loop of `Functor.contract` as a public `Functor.operands`
- [ ] Add a `contract` parameter to `tensor.Functor` and `Diagram.eval` choosing the
      engine between `einsum` (default, switching to `opt_einsum` past
      `config.MAX_EINSUM_INDICES`) and `quimb`
- [ ] Build the quimb network from the operands: `Functor.to_quimb` and `CMap.to_quimb`,
      with `Diagram.to_quimb` delegating through `to_map()`
- [ ] Thread `contract` through `quantum.circuit.Circuit.eval` and `Sum.eval`
- [ ] Declare the contraction dependencies: a `tensor` extra with `opt_einsum`, `quimb`
      and `cotengra`; fix the README description of quimb
- [ ] Tests: engines agree on the `CMap` diagram zoo, `CMap.to_quimb` vs `eval`,
      trailing-swap regression (#297), compressed contraction, autodiff through quimb,
      repr round-trip
- [ ] `CHANGELOG.md` entry, `pflake8` and `coverage run -m pytest` green
