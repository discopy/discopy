...
- [ ] `serialisation` — `from_tree` asks `discopy.hopf` for an attribute named
...
- [ ] `transparency` — `NameError: name 'complex128' is not defined`. A `tensor.Box` subscripted
...
- [ ] `reidemeister_1_cap` and `reidemeister_1_cup` — both recorded counterexamples now **fail
...
- [ ] `pflake8 discopy` and the suite green before the push.
