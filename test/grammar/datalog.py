...
def montague():
    ...
    lexicon = Lexicon(
        ...
        ar_map={every: EVERY, a: A, woman: WOMAN, man: MAN,
            married: married_sem})
...
def test_from_lexicon_errors():
    ...
    e, t = Ty("e"), Ty("t")
    infinite = Lexicon(ob_map={n: e}, ar_map=lambda w: e(w.name))
    with raises(ValueError):
        Program.from_lexicon(infinite)
    clashing = Lexicon(ob_map={n: e >> t}, ar_map={n("woman"): (e >> t)("n")})
    with raises(ValueError):
...
def test_seminaive():
    ...
    lexicon = Lexicon(
        ob_map={n: ET, s: t},
        ar_map={woman: WOMAN, lady: WOMAN, unary: ET(lambda p: p)})
    program = Program.from_lexicon(lexicon)
...
def test_parse_string_cfg():
    ...
    lexicon = Lexicon(
        ob_map={n: STR, np: STR, s: STR},
        ar_map={John: o(lambda z: john_c(z)),
...
def ambiguous():
    ...
    WOMAN = (e >> t)("WOMAN")
    woman, lady = n("woman"), n("lady")
    lexicon = Lexicon(ob_map={n: e >> t, s: t}, ar_map={woman: WOMAN, lady: WOMAN})
    program = Program.from_lexicon(lexicon)
    facts, _ = program.database(WOMAN)
    forest = program.seminaive(facts)
    rules = {rule.word.name: rule for rule in program.rules}
...
def test_weighted_cyclic():
    ...
    lexicon = Lexicon(
        ob_map={n: ET, s: t},
        ar_map={woman: WOMAN, lady: WOMAN, very: ET(lambda p: p)})
    program = Program.from_lexicon(lexicon)
    facts, _ = program.database(WOMAN)
    forest = program.seminaive(facts)
    goal = Atom('n', (0, 1))
...
def arithmetic():
    ...
    syntax = Lexicon(ob_map={T: STR, E: STR}, ar_map=syntax_ar)
    semantics = Lexicon(ob_map={T: CH, E: CH}, ar_map=semantics_ar)
