...
4. **An agent talks only to its own human.** Never reply to another user's comment unless the
...
5. **`[x]` marks agent-done; deleting `TODO.md` is the human sign-off.** An agent deletes it only
