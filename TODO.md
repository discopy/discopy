...
- [WIP] @session_01Ux5oPZt3xhLRBUZBAZTQ4f-2026-09-15 12:00 Rename the planar traced module `traced.py` to `planar.py`, re-pointing `balanced`, `pivotal`, `interaction`, `cmap`, tests and docs, keeping the doctest image paths.
- [ ] Make the diagram hierarchy follow `abc`: `symmetric.Diagram` inherits from `braided.Diagram`, dropping the trace and twist it inherited through `balanced`; `compact` keeps the identity twist; `markov` loses its `Trace`.
