...
def test_axioms():
    ...
    def hypergraph_equal(left, right):
        return left.to_hypergraph() == right.to_hypergraph()

    assert hypergraph_equal(Id(x).transpose(), Id(x))
    assert hypergraph_equal(Id(x).transpose(left=True), Id(x))
    assert hypergraph_equal(Cap(x, x.r) >> Swap(x, x.r), Cap(x.r, x))
    assert hypergraph_equal(Swap(x, x.r) >> Cup(x.r, x), Cup(x, x.r))
