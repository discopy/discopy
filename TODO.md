...
- [WIP] @session_01Ux5oPZt3xhLRBUZBAZTQ4f-2026-09-16 14:00 Replace `delay(n_steps)` by the abstract property `d` on `abc.DelayedMonoid` and `abc.FeedbackCategory`, trivial on `abc.TracedCategory` and `monoidal.Ty`, with the `time_step` arithmetic of `feedback` going through constructors.
