...
def solve_sudoku(grid: tuple[int, ...], n: int | tuple[int, int] = 2,
                 ...
                 shuffle: random.Random = None) -> tuple[int, ...] | None:
    """
    ...
    Parameters:
        ...
        shuffle : A random generator reshuffling the digit order at every
                  cell, so that solving the empty grid samples a random
                  completed grid rather than following one fixed order.
...
def random_sudoku(n: int | tuple[int, int] = 2, n_clues: int = 8,
    ...
    the digit order reshuffled at every cell and then blanking random cells.
    ...
    solution = solve_sudoku(size * size * (0, ), n, shuffle=generator)
