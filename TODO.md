...
- [ ] `Axiom` is a law of an element and `ClassAxiom` a law of the carrier, told apart by the decorator — `axiom` and `classaxiom`, sugar for `axiom` of a `classmethod` — instead of the name of the first parameter; `abc.py` and `cat.py` declare their laws with `classaxiom`
- [ ] Document every attribute of `Axiom`, say what a carrier is, and give `modulo` an example that makes sense of a `Diagram`
- [ ] Rename `discopy.testing` to `discopy.axiom` — module, tests, docs page, `AGENTS.md`, `CONTRIBUTING.md`, `CHANGELOG.md` — and move `Equation` into it
- [ ] Open an issue for `abc.DaggerCategory`, so a functor need not declare the dagger laws inapplicable
- [ ] Run `pflake8` and the test suite, update the PR body, reply on and resolve every thread
