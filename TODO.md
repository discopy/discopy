...
- [ ] Mint the App token in `no-todo-on-main.yml` so `convertPullRequestToDraft` is reachable
- [ ] Fail the job unless the pull request really came back draft, so the gate can never exit 0
      while a TODO file is present on a non-draft pull request
- [ ] Drop `pull-requests: write` from a `pull_request_target` workflow
