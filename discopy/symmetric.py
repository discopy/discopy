...
class Diagram(balanced.Diagram, SymmetricCategory, metaclass=Swappable):
    ...
    Note
    ----
    The swaps of atomic types are generated by :attr:`braid_factory`, which
    subclasses should set to their own subclass of :class:`Swap`. Thanks to
    the metaclass :class:`Swappable`, it can also be set under the alias
    ``swap_factory``, i.e. the following two lines are equivalent:
    ...
    def swap(cls, left: monoidal.Ty, right: monoidal.Ty) -> Diagram:
        ...
        also accessible under the alias ``swap_factory``.
        """
