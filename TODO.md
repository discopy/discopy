...
- [ ] Reproduce the misplacement with a `Controlled` gate over classical wires
- [ ] Select the controlled sub-gate's x-coordinate from the wire it actually sits on instead of `box.dom[0]`
- [ ] Compare `draw(to_tikz=True)` against the matplotlib output and adjust `bend_in`/`bend_out` for the TikZ backend
- [ ] Add regression tests and regenerate the affected docs images
