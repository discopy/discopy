...
- [test](test/) is flat with one file for each module, mirroring `discopy/`; submodules (`grammar`, `quantum`, `drawing`) keep their own directory with one file per submodule, and shared test data lives in `fixtures`
