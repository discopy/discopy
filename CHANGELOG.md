...
- `discopy.python.additive.Function` joins the property matrix, with a
  ...
  can name it, and has no tree. The objects come from a new
  `python.function.Types`, the subclass of `python.Ty` — i.e. of
  `List[type]` — that generates itself: the matrix quantifies over the
  one-type universe of `int`, so a function is drawn over objects it can
  build elements of, and `python.additive.Ty` is that subclass rather than
  the bare free monoid.
