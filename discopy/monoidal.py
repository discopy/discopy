...
class Trace(Bubble):
    """
    A trace is a bubble with an output wire of ``arg`` fed back into an
    input, the syntax generated by :meth:`discopy.abc.TracedCategory.trace`.

    It lives here as pure syntax, next to :class:`Bubble`: monoidal diagrams
    themselves have no trace, the traced levels of the hierarchy subclass it
    and set it as their ``trace_factory``, e.g. :class:`balanced.Trace` and
    :class:`traced.Trace`; :mod:`pivotal` instead derives its trace from
    cups and caps.
    ...
    def __init__(self, arg: Diagram, left=False):
        ...
        Bubble.__init__(self, arg, dom=dom, cod=cod)
...
class Functor(cat.Functor):
    ...
    def __call__(self, other):
        ...
        if isinstance(other, Layer):
            ...
            return result
        if isinstance(other, Trace) and hasattr(self.cod, "trace"):
