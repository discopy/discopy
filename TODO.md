...
- [x] `axioms.Goal`, a pattern sequent over goal variables with an environment and a fuel, a side left open being a hole; `align` unifying a rule's conclusion with a goal, splitting holes and binding what a proof determines; premises proved in sequence with the environment threaded, the most constrained first, a closed goal drawing its middle from the types and the hints as before; the box, the permutation and a constant on goals; `search` from open boundaries; docs, CHANGELOG, tests, lint, `uv run pytest`, the fast matrix
