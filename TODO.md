...
- [WIP] @session_01Ux5oPZt3xhLRBUZBAZTQ4f-2026-09-09 12:10 Give traced types the trivial delay concretely: `monoidal.Ty.delay` is the identity like `unwind`, overridden by `feedback.Ty`; document the new methods in `traced`. The free `feedback.Diagram` and `feedback.Ty` cannot conversely become base classes of `traced.Diagram` and `traced.Ty`: they are already their subclasses through `markov`, so the concrete inheritance goes the other way around and the shared interface lives in `abc`.
