...
- a `Layer(*inside)` is a tensor product of `Box` and non-empty `Ty` values with at least one box and no adjacent types; construction coalesces adjacent types, while whiskering `X @ f` and `f @ X` extends or merges the routing at either boundary
