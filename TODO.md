...
- [x] Drop the `ob = Ty` on `Curry`, `Trace` and `Sum` if the factory closure holds without them, answer both threads.
- [x] `Lexicon` is a functor between abstract grammars (de Groote 2001, §2.2): atomic types to types, constants to terms; the string vocabulary of §4 (one atomic type, words as constants of type `* >> *`, concatenation as composition) and the string lexicon a categorial grammar comes with.
- [x] Replace `test_python_Functor_on_terms` with the finite-universe test: random predicates, the two sentences compile to the same truth value as the Python expressions.
- [x] Reproduce and fix `eval()` on the semantic lexicon's non-linear images, the new test evaluates them.
- [x] Reproduce and fix `is_linear` on `Curry`, `Trace`, `Sum` and on terms.
- [x] `CHANGELOG.md`, docstrings, `pflake8` and the full suite green, then delete `TODO.md`.
